...
namespace sakura
{
	...
	public:
		...
		/// <summary>
		/// Start the clock with the current CPU time in cycle, as returned by the high resolution timer.
		...
		time::Seconds delta_time_seconds() const { return time::cycles_to_seconds(delta_time_cycles(), cycles_per_second_); }
